...
var UserModule = (function (_super) {
    ...
    UserModule.prototype.init = function () {
        ...
        var communications = this.getModuleManager().get("communication");
    ...
    UserModule.prototype.passwordRecovery = function (email, callback) {
        //Open DB connection
        this.db.getConnection(function (err, connection, cleanup) {
            if (err) {
                return callback(err);
            ...
            async.waterfall([
                ...
                function getSaltWithEmail(next) {
                    ...
                    connection.query("SELECT salt FROM user WHERE email = ?", [email], function (err, rows) {
                        var myError = null;
                        if (rows.length === 0) {
                            myError = new utils.errors.ApplicationError(null, {}, "Email doesn't belong to any user");
                        }
                        if (err) {
                            myError = new utils.errors.DatabaseError(err);
                        }
                        next(myError, rows.length === 1 && rows[0].salt);
                    });
                },
