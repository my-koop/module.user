...
var UserModule = (function (_super) {
    ...
    UserModule.prototype.updatePermissions = function (id, newPermissions, callback) {
        ...
        this.db.getConnection(function (err, connection, cleanup) {
            ...
            var query = connection.query("UPDATE user SET perms = ? WHERE id = ?", [UserModule.serializePermissions(newPermissions), id], function (err, rows) {
                cleanup();
                return callback(err, !err && rows.affectedRows === 1);
            } //function
            ); //update query
            ...
            async.waterfall([
                function getSaltWithEmail(next) {
                    ...
                    connection.query("SELECT salt FROM user WHERE email = ?", [email], function (err, rows) {
                        ...
                        next(rows.length === 0 && new utils.errors.ApplicationError(null, {}, "Email doesn't belong to any user"), rows[0].salt);
                    });
                },
                ...
                function updateUserPassword(password, passwordHash, next) {
                    ...
                    connection.query("UPDATE user SET pwdhash = ? WHERE email = ?", [passwordHash, email], function (err, rows) {
                        var myError = null;
                        if (err) {
                            next(new utils.errors.DatabaseError(err, "Databse error while updating user password"), null);
                        }
                        logger.debug(rows, "Result of password update");
                        next(rows.affectedRows !== 1 && new utils.errors.ApplicationError(null, {}, "Update request did not affect change to user row"), password);
                    });
                },
