...
var UserModule = (function (_super) {
            ...
            async.waterfall([
                ...
                function updateUserPassword(password, passwordHash, next) {
                    ...
                    connection.query("UPDATE user SET pwdhash = ? WHERE email = ?", [passwordHash, email], function (err, rows) {
                        var myError = null;
                        if (err) {
                            next(new DatabaseError(err, "Database error while updating user password"));
                        }
                        logger.debug(rows, "Result of password update");
                        next(rows.affectedRows !== 1 && new ApplicationError(null, {}, "Update request did not affect change to user row"), password);
                    });
                },
                function sendEmail(password, next) {
                    ...
                    var emailParams = {
                        subject: "CoopB�cik - New Password / Nouveau mot de passe",
    ...
    UserModule.prototype.__newNote = function (connection, params, callback) {
        var query = connection.query("INSERT INTO user_notes SET ?", [params], function (err, res) {
