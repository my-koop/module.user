...
var validatePermissions = require("./common/validatePermissions");
...
var UserModule = (function (_super) {
    ...
    UserModule.validateCurrentUser = function (req, callback) {
        callback((!req.session.user || req.session.user.id !== parseInt(req.param("id"))) && new Error("Not current user."));
    };
    ...
    UserModule.prototype.getProfile = function (params, callback) {
        this.callWithConnection(this.__getProfile, params, callback);
    };
    UserModule.prototype.__getProfile = function (connection, params, callback) {
        connection.query("SELECT ?? FROM user WHERE id = ?", [UserProfile.COLUMNS, params.id], function (err, rows) {
            if (err) {
    ...
    UserModule.prototype.updatePermissions = function (id, newPermissions, callback) {
        ...
        this.db.getConnection(function (err, connection, cleanup) {
            ...
            connection.query("UPDATE user SET perms = ? WHERE id = ?", [UserModule.serializePermissions(newPermissions), id], function (err, rows) {
                ...
                connection.query("DELETE FROM sessions WHERE data LIKE " + "'%\"id\":" + +id + "%'", function (err, res) {
                    cleanup();
                    if (err) {
                        logger.error(err);
                    }
                    return callback(err && new DatabaseError(err), !err && rows.affectedRows === 1);
                });
            } //function
            ); //update query
        }); //getConnection
    ...
    UserModule.prototype.__userActivation = function (connection, params, callback) {
        ...
        connection.query("UPDATE user SET deactivated=? WHERE id=?", [activation, params.id], function (err, res) {
            ...
            if (!params.activate) {
                connection.query("DELETE FROM sessions WHERE data LIKE " + "'%\"id\":" + +params.id + "%'", function (err, res) {
                    if (err) {
                        logger.error(err);
                    }
                    return callback(err && new DatabaseError(err), { isActive: +params.activate });
                });
                return;
            }
